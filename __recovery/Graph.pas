...
Type
  ...
  public
    ...
    procedure PaintExtremaDots(var Bitmap: TBitmap; Scale, XFrom, XTo, YTo, YOffset, XOffset: Integer);
...
implementation
  ...
  procedure TGraph.PaintExtremaDots(var Bitmap: TBitmap; Scale, XFrom, XTo, YTo, YOffset, XOffset: Integer);
  const
    CIRCLE_RADIUS = 5;
      ...
      while (CurrNode <> nil) do
        Begin
          if (CurrNode.X >= XFrom)  and (CurrNode.X <= XTo) then
            Begin
              X := Round(Scale * CurrNode.X) + XOffset;
